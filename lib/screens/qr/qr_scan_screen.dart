...
class _QrScanScreenState extends State<QrScanScreen> {
  ...
  Future<void> _submitCode(String code) async {
    if (_processing || (code = code.trim()).isEmpty) return;
    setState(() {
      _processing = true;
      _message = null;
    });
    try {
      await _api.scanQr(code);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Scan successful')));
        Navigator.pop(context);
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          _message = 'Invalid or expired code';
          _processing = false;
        });
      }
    }
